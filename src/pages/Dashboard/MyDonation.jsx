...
const MyDonation = () => {
  ...
  const { data: campaign = [] } = useQuery({
    queryKey: ["camapaign", user?.email],
    queryFn: async () => {
      const { data } = await axiosCommon.get(`/campaign/${user?.email}`);
      console.log(data);
      return data;
    },
  });
  ...
  const { mutateAsync } = useMutation({
    mutationFn: async (id) => {
      const { data } = axiosCommon.delete(`/campaign/${id}`);
      return data;
    },
    onSuccess: (data) => {
      console.log(data);
    ...
    <section className="container px-2 mx-auto pt-12">
      ...
      <div className="flex flex-col mt-6">
        <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-contents py-2 align-middle md:px-6 lg:px-8">
